...
function parseMethod({
  ...
  if (Parameter) {
    metadata.Parameter = {};
    for (const { $: { Name, Type, Nullable, /*, Unicode*/ } } of Parameter) {
      metadata.Parameter[Name] = parseTypeReference(Type, Nullable);
    }
    if (metadata.IsBound) {
  ...
  if (ReturnType) {
    metadata.ReturnType = parseTypeReference(ReturnType[0].$.Type);
  }
  return metadata;
}
